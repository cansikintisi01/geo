...
function drawSpeedLines(ctx) {
    for(let i = speedLines.length - 1; i >= 0; i--) {
        ...
        line.life -= clock.getDelta();
        
        const alpha = line.life / 0.3;
        ctx.strokeStyle = line.color;
        ctx.globalAlpha = alpha;
        ctx.lineWidth = line.width;
        ctx.beginPath();
        ctx.moveTo(line.x, line.y);
        ctx. lineTo(line.x + line.speed * (0.3 - line.life), line.y);
        ctx.stroke();
        
        if(line.life <= 0) speedLines.splice(i, 1);
    }
    ctx.globalAlpha = 1;
}
...
function updatePhysics(dt) {
    ...
    // ✅ FOV FIX - Çok hızlandığında aşırı zoom out olmuyor! 
